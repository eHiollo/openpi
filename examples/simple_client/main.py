...
def main(args: Args) -> None:
    obs_fn = {
        ...
        EnvMode.A10: _random_observation_a10,
    }[args.env]
...
def _random_observation_a10() -> dict:
    """Fake A10 observation matching the robot-side VLA client / A10Inputs."""
    return {
        "observation/state": np.zeros((1, 7), dtype=np.float32),
        "observation/images/right": np.random.randint(256, size=(3, 224, 224), dtype=np.uint8),
        "observation/images/top": np.random.randint(256, size=(3, 224, 224), dtype=np.uint8),
        "prompt": "Reach the target fruit on the table.",
    }
